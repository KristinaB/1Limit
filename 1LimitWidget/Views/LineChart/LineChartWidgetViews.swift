...
struct LineChartSmallWidgetView: View {
...
struct LineChartMediumWidgetView: View {
            ...
            HStack {
                ...
                if let priceChange = getPriceChange() {
                    VStack(alignment: .trailing, spacing: 1) {
                        Text("\(priceChange.percentage >= 0 ? "+" : "")\(priceChange.percentage, specifier: "%.2f")%")
                            .font(.caption2)
                            .fontWeight(.medium)
                            .foregroundColor(priceChange.percentage >= 0 ? .green : .red)
                        
                        Text("\(priceChange.absolute >= 0 ? "+" : "")\(priceChange.absolute, specifier: "%.4f")")
                            .font(.caption2)
    ...
    private func getPriceChange() -> (absolute: Double, percentage: Double)? {
        guard let first = entry.priceData.first,
              let last = entry.priceData.last else {
            return nil
        }

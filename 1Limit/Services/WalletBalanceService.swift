...
struct TokenBalanceInfo {
  ...
  var formattedBalance: String {
    ...
    if readableAmount >= 1_000_000 {
      return String(format: "%.2fM", readableAmount / 1_000_000)
...
class WalletBalanceService: ObservableObject {
  ...
  func fetchWalletBalance(for walletAddress: String, forceRefresh: Bool = false) async {
    guard !walletAddress.isEmpty else {
      await updateError(.invalidWalletAddress)
      return
    }

    // Prevent excessive calls - limit to once every 5 seconds (unless forced)
    if !forceRefresh {
      let now = Date()

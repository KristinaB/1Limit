...
class BundledUITests: XCTestCase {
    ...
    func testNavigationAndTabBarBundle() throws {
        ...
        if useTestWalletButton.exists {
            ...
            usleep(1_000_000)  // 1 second for wallet to load
        }
        ...
        if amountField.exists {
            ...
            if let currentValue = amountField.value as? String {
                XCTAssertEqual(
                    currentValue, "1.5", "Trade form data should persist across navigation")
            }
        }
    ...
    func testTradeFormInteractionsBundle() throws {
        ...
        if amountField.exists {
            ...
            XCTAssertEqual(
                amountField.value as? String, "2.5", "Amount field should show entered value")
        }
        ...
        if limitPriceField.exists {
            ...
            XCTAssertEqual(
                limitPriceField.value as? String, "0.85",
                "Limit price field should show entered value")
        }
        ...
        if createOrderButton.exists {
            ...
            if !createOrderButton.isEnabled {
                XCTAssertFalse(
                    createOrderButton.isEnabled, "Button should be disabled with empty fields")
            }
        }
        ...
        if chartButton.exists {
            ...
            if chartView.waitForExistence(timeout: 3) {
                ...
                XCTAssertTrue(
                    createOrderTitle.waitForExistence(timeout: 3), "Should return to trade view")
            }
        }
    ...
    func testWalletFlowBundle() throws {
        ...
        if !activeWalletText.exists {
            XCTAssertTrue(
                createWalletButton.exists,
                "Create New Wallet button should exist when no wallet is active")
            XCTAssertTrue(
                testWalletButton.exists,
                "Use Test Wallet button should exist when no wallet is active")
            XCTAssertTrue(
                createWalletButton.isHittable, "Create New Wallet button should be tappable")
        } else {
    ...
    func testContentVerificationBundle() throws {
        ...
        if transactionsTab.exists {
            ...
            if allFilter.exists {
                allFilter.tap()
                XCTAssertTrue(
                    allFilter.isSelected || allFilter.label.contains("All"),
        ...
        if createOrderButton.exists && createOrderButton.isEnabled {
            ...
            if confirmTitle.waitForExistence(timeout: 5) {
                ...
                if cancelButtons.count > 0 {
                    ...
                    XCTAssertTrue(
                        tradeTitle.waitForExistence(timeout: 3), "Should return to trade view")
                }
            }
        }
